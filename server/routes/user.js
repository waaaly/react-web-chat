...
router.post('/setAvatar', async (req, res) => {
    ...
    let reqJson = req.body;
    let nickName = reqJson.nickName,
        avatar = reqJson.avatar;

    UserInfo.updateOne({ nickname:nickName }, { $set: { avatar } })
    PrivateRecords.updateMany({ nickname:nickName }, { $set: { avatar } })
    resJson.message = "头像修改成功，请尝试刷新页面。"
    res.json(resJson);
})
