...
const sendToPrivate = async ({userId, toId, avatar, nickname, message, time,type}) => {
  await async.parallel({
    privateRecords: function (callback) {
      PrivateRecords.create(
        {'fromId': userId, 'toId': toId, 'nickname': nickname, 'avatar': avatar, 'message': message, 'time': time,'type':type},
        function (err) {
          callback(err);
        });
    },
...
const clearGroupUnreadMessage = async (id) => {
  return new Promise((resolve, reject) => {
    UserGroupRelation.deleteOne({'userId': id}, (err, ducs) => {
...
const clearPrivateUnreadMessage = async ({userId, fromId}) => {
  return new Promise((resolve, reject) => {
    UserUserRelation.deleteOne({$and: [{'userId': userId}, {'fromId': fromId}]}, (err, ducs) => {
      if (err) return reject(err);
      return resolve();
    });
  });
...
const logout = async ({userId}) => {
  return new Promise((resolve, reject) => {
    UserInfo.updateOne({'userId': userId}, {'isOnline': false}, function (err, docs) {
      if (err) return reject(err);
      return resolve();
    });
  });
